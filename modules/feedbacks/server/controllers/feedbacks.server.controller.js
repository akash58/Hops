...
exports.create = function(req, res) {
  var feedback = new req.db.Feedback(req.body);
...
exports.list = function(req, res) {
  req.db.Feedback.find().sort('-created').populate('user', 'displayName').exec(function(err, feedbacks) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.feedbackByID = function(req, res, next, id) {
  ...
  req.db.Feedback.findById(id).populate('user', 'displayName').exec(function (err, feedback) {
    if (err) {
      return next(err);
    } else if (!feedback) {
